...
async function loadMembers(){
  ...
  try{
    ...
    grid.innerHTML = '';
    if(snap.empty){ grid.innerHTML = '<p style="color:var(--ink-300)">No members found.</p>'; return; }
    let count = 0;
    snap.docs.forEach(d=>{
      const m = d.data();
      ...
      if(gradeVal) {
        const gradeNum = 12 - (Number(year) - (new Date().getMonth() >= 6 ? new Date().getFullYear()+1 : new Date().getFullYear()));
        if(String(gradeNum) !== gradeVal) return;
      }
  ...
  const card = document.createElement('article');
      card.className = 'member-card' + (isOfficer ? ' officer' : '');
      const avatarHtml = m.avatarUrl
        ? `<img src="${m.avatarUrl}" alt="${escapeHtml(name)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%" />`
        : escapeHtml(initials);
      card.innerHTML = `
        <div class="member-avatar-sm" aria-hidden="true">${avatarHtml}</div>
        <div style="flex:1;min-width:0">
          <div class="member-name">${escapeHtml(name)}</div>
          <div class="member-grade">${metaText || (gradeLabel ? escapeHtml(gradeLabel) : '')}</div>
          ${points > 0 ? `<div class="member-points">${points} pts</div>` : ''}
        </div>
      `;
    ...
    if(!count) grid.innerHTML = '<p style="color:var(--ink-300)">No members match your search.</p>';
  }catch(err){ console.error('members-public load failed', err); grid.innerHTML = '<p style="color:var(--ink-300)">Failed to load members.</p>'; }
}
...
if(gradeFilter) gradeFilter.addEventListener('change', ()=>{ loadMembers(); });
